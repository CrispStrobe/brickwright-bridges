...
try:
    import websockets  # noqa: F401 — used inside _start_server_for_device
except ImportError:
    ...
    def _check_all(self):
        ...
        try:
            if hasattr(std_socket, "AF_BLUETOOTH"):
                ...
                self._logger.warning(
                    "  ⚠️  AF_BLUETOOTH not available - Native BT disabled"
                )
        ...
        if platform.system() == "Darwin":
            try:
                ...
                self._logger.warning(
                    "  ⚠️  PyObjC missing - macOS Bluetooth scan might hang/fail"
                )
                self._logger.warning(
                    "      Install: pip install pyobjc-framework-IOBluetooth"
                )
...
class ProtocolTransformer:
    ...
    def _parse_nxt(data: bytes) -> Dict[str, Any]:
        ...
        if cmd_type == 0x02:
            if opcode == 0x0B and len(data) >= 7:  # Battery level
                ...
                return {"event": "sensor_update", "port": port, "value": value}
    ...
    def _parse_boost(data: bytes) -> Dict[str, Any]:
        ...
        # Hub attached IO
            ...
            return {"event": "port_change", "port": port, "attached": event == 1}
            ...
            if text.startswith("{"):
                ...
                if "m" in json_data:
                    ...
                    if msg_type == 0:  # Hub status
                        return {"event": "hub_status", "data": json_data.get("p", {})}
                    elif msg_type == 2:  # Battery
                        battery = json_data.get("p", [0, 0])[1]
                        return {"event": "battery_update", "percent": battery}
                        ...
                        return {"event": "gesture", "type": json_data.get("p")}
    ...
    def _encode_nxt(command: Dict[str, Any]) -> bytes:
        ...
        if cmd_type == "motor_power":
            ...
            payload = bytes(
                [0x80, 0x04, port, power, 0x01, 0x00, 0x20, 0x00, 0x00, 0x00, 0x00]
            )
            length = len(payload)
...
class ConnectionStats:
    ...
    def __str__(self):
        ...
        return (
            f"{self.device_type.value.upper()}: "
...
class BaseConnection:
    ...
    def __init__(
        ...
        protocol_mode: ProtocolMode = ProtocolMode.RAW,
    ):
    ...
    async def reconnect_loop(self):
        ...
        while self.reconnect_attempts < self.max_reconnect_attempts:
            self.logger.info(
                f"🔄 Reconnection attempt {self.reconnect_attempts + 1}/{self.max_reconnect_attempts}"
            )
    ...
    async def connect(self) -> bool:
        ...
        try:
            self.serial = DEPS.serial_lib.Serial(
                self.device_info.port, baudrate=self.baudrate, timeout=0.1
            )
                    ...
                    if self.device_info.device_type == DeviceType.NXT:
                        ...
                        if packet:
                            ...
                            asyncio.run_coroutine_threadsafe(
                                self.read_queue.put(packet), asyncio.get_event_loop()
                            )
                        ...
                        asyncio.run_coroutine_threadsafe(
                            self.read_queue.put(chunk), asyncio.get_event_loop()
                        )
                else:
                    time.sleep(0.01)
        ...
        if DEPS.features["classic_native"] and platform.system() != "Darwin":
            try:
                ...
                await loop.run_in_executor(
                    None,
                    lambda: self.socket.connect(
                        (self.device_info.address, self.channel)
                    ),
                )
    ...
    def _read_loop(self):
        ...
        while self.running:
            try:
                ...
                if len(data) < length:
                    self.logger.warning(
                        f"⚠️  Incomplete packet: got {len(data)}, expected {length}"
                    )
                    continue
...
class BLEConnection(BaseConnection):
    ...
    def __init__(
        ...
        self.char_uuid = self.DEVICE_CHARACTERISTICS.get(
            ...
            (
                list(device_info.characteristics.values())[0]
        ...
        if not self.char_uuid:
            raise ValueError(
                f"No BLE characteristic found for {device_info.device_type}"
            )
    ...
    async def _discover_serial(self):
        ...
        try:
            ...
            ports = await loop.run_in_executor(
                None, DEPS.serial_lib.tools.list_ports.comports
            )
            ...
            for port in ports:
                ...
                if "nxt" in name_lower:
                    dev_type = DeviceType.NXT
                elif "ev3" in name_lower:
                    dev_type = DeviceType.EV3
                elif "spike" in name_lower or "lego hub" in name_lower:
                    dev_type = DeviceType.SPIKE_PRIME
                elif (
                    "usb serial" in name_lower
                    and port.manufacturer
                    and "lego" in port.manufacturer.lower()
                ):
                    dev_type = DeviceType.EV3
            ...
            if platform.system() == "Darwin":
                try:
                    await asyncio.wait_for(
                        DEPS.bleak_scanner.discover(timeout=0.5), timeout=1.0
                    )
                except asyncio.TimeoutError:
                    pass
                except Exception as e:
                    if (
                        ...
                        self.logger.error(
                            "❌ BLE Permission Denied! Enable Bluetooth in System Settings."
                        )
                        return devices
...
class ConnectionFactory:
    ...
    @staticmethod
    def create(
        device_info: DeviceInfo, logger: logging.Logger, protocol_mode: ProtocolMode
    ) -> BaseConnection:
        ...
        if device_info.connection_type == ConnectionType.SERIAL:
            ...
            return UniversalBluetoothClassicConnection(
                device_info, logger, protocol_mode
            )
        elif device_info.connection_type == ConnectionType.BLE:
            return BLEConnection(device_info, logger, protocol_mode)
        else:
            raise ValueError(
                f"Unsupported connection type: {device_info.connection_type}"
            )
...
class WebSocketRelay:
    ...
    async def handle_client(self, websocket):
        ...
        self.logger.info(
            f"📱 Client connected: {client_id} ({self.connection.device_info.device_type.value})"
        )
                ...
                if data:
                    ...
                    if self.connection.protocol_mode == ProtocolMode.NORMALIZED:
                        normalized = ProtocolTransformer.to_normalized(
                            self.connection.device_info.device_type, data
                        )
                        message = json.dumps(normalized)
                    else:
                        # RAW mode
                        if self.connection.device_info.device_type in [
                            DeviceType.NXT,
                            DeviceType.BOOST,
                            DeviceType.WEDO2,
                        ]:
                ...
                if self.connection.protocol_mode == ProtocolMode.NORMALIZED:
                    ...
                    try:
                        ...
                        data = ProtocolTransformer.from_normalized(
                            self.connection.device_info.device_type, command
                        )
                    except Exception as e:
...
class UniversalBridgeServer:
    ...
    async def add_device(self, device_info: DeviceInfo) -> bool:
        ...
        try:
            connection = ConnectionFactory.create(
                device_info, self.logger, self.protocol_mode
            )
    ...
    async def _start_server_for_device(self, port: int, device_id: str):
        ...
        try:
            ...
            self.logger.info(
                f"🎉 Server started: ws://0.0.0.0:{port} ({connection.device_info.device_type.value.upper()})"
            )
        except Exception as e:
        ...
        while self.running:
            ...
            if DEPS.features["serial"]:
                try:
                    new_serial_devices = await asyncio.wait_for(
                        self.discovery._discover_serial(), timeout=3.0
                    )
                    ...
                    for device in new_serial_devices:
                        ...
                        if not is_connected:
                            self.logger.info(
                                f"🆕 Serial device detected: {device.name}"
                            )
            ...
            if (current_time - last_ble_scan) > BLE_SCAN_COOLDOWN:
                ...
                if consecutive_ble_failures >= 3:
                    self.logger.debug(
                        f"⏸️  BLE scan paused ({consecutive_ble_failures} failures)"
                    )
                ...
                try:
                    ...
                    if DEPS.features["ble"]:
                        new_devices.extend(
                            await asyncio.wait_for(
                                self.discovery._discover_ble(), timeout=15.0
                            )
                        )

                    # Classic BT (non-macOS only)
                    if (
                        DEPS.features["classic_legacy"]
                    ...
                    self.logger.warning(
                        f"⚠️  Bluetooth scan timeout ({consecutive_ble_failures})"
                    )
                except Exception as e:
                    consecutive_ble_failures += 1
...
╚═══════════════════════════════════════════════════════════════════════╝
    ...
    logger.info(
        f"🔄 Auto-Reconnect: {'Enabled' if args.auto_reconnect else 'Disabled'}"
    )
    ...
    if not any(DEPS.features.values()):
        ...
        logger.erorr(
            "   or: pip install git+https://github.com/airgproducts/pybluez2.git"
        )
        return
    ...
    protocol_mode = (
        ...
        if args.protocol_mode == "normalized"
        else ProtocolMode.RAW
    )
    ...
    for port, device_id in server.port_mapping.items():
        ...
        logger.info(
            f"📡 ws://localhost:{port} → {connection.device_info.device_type.value.upper()}"
        )
...
Examples:
  ...
  %(prog)s --stats-interval 30                # Stats every 30s
    ...
    parser.add_argument(
        "--no-hot-swap", action="store_true", help="Disable hot-swap device detection"
    )
    parser.add_argument("--stats", action="store_true", help="Print statistics on exit")
    parser.add_argument(
        "--stats-interval",
        type=int,
        default=0,
